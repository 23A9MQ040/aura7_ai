...
import { Shield, Lock, Eye, FileText, Trash2, Download, ToggleLeft, ToggleRight, AlertTriangle, CheckCircle2, Clock, Key, RotateCw } from 'lucide-react';
...
const initialAuditLogs = [
  { time: '2 min ago', action: 'Memory sync completed', agent: 'Memory Agent', type: 'success' },
  { time: '15 min ago', action: 'Data encryption verified', agent: 'Privacy Guardian', type: 'success' },
  { time: '1 hr ago', action: 'Permission check passed', agent: 'Auth Service', type: 'success' },
  { time: '2 hr ago', action: 'New consent recorded', agent: 'Privacy Guardian', type: 'info' },
  { time: '6 hr ago', action: 'Scheduled security scan — all clear', agent: 'Privacy Guardian', type: 'success' },
  { time: '1 day ago', action: 'API key rotated automatically', agent: 'Auth Service', type: 'info' },
];
...
export default function PrivacyPage() {
  ...
  const rotateKey = () => {
    ...
    setTimeout(() => {
      ...
      setLogs(prev => [
        {
          time: 'Just now',
          action: 'Client API key manually rotated',
          agent: 'Auth Service',
          type: 'success',
        },
        ...prev
  ...
  const handleExport = () => {
    ...
    downloadAnchor.setAttribute('download', 'aura_privacy_profile.json');
  ...
  const handleDeleteEverything = () => {
    if (window.confirm('WARNING: Are you sure you want to delete all cached profiles and historical audit logs? This action is irreversible.')) {
      setPerms(prev => prev.map(p => ({ ...p, enabled: false })));
      setLogs([
        {
          time: 'Just now',
          action: 'All profile and usage data erased',
  ...
  // Calculate dynamic security score: less data shared = higher score
  const securityScore = Math.min(100, 100 - (enabledCount * 5) + (logs.length > 5 ? 10 : 0));
      ...
      <div className="space-y-8">
        ...
        <div className="grid md:grid-cols-3 gap-5">
          <GlowCard color="cyan" className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-xs text-aura-muted">Score: {securityScore}%</p>
          ...
          <GlowCard color="green" className="relative group">
            <div className="flex items-center justify-between mb-2">
              ...
              <button 
                onClick={rotateKey}
                disabled={rotating}
                className="p-1 rounded bg-white/5 text-aura-muted hover:text-aura-cyan hover:bg-white/10 transition-colors disabled:opacity-50"
                title="Rotate Access Key"
              >
                <RotateCw className={`w-3.5 h-3.5 ${rotating ? 'animate-spin text-aura-cyan' : ''}`} />
              </button>
            </div>
            ...
            <div className="flex items-center gap-2 mt-3">
              ...
              <span className="text-[10px] text-aura-green">Session token active</span>
            </div>
          </GlowCard>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="glass-card p-4 space-y-3 max-h-[460px] overflow-y-auto custom-scrollbar">
              <AnimatePresence>
                {logs.map((log, i) => (
                  <motion.div
                    key={log.action + i}
        ...
        <div className="glass-card p-6 border-aura-red/20">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div className="glass rounded-xl p-4">
              ...
              <button onClick={handleExport} className="btn-secondary text-xs !py-2 flex items-center gap-2 w-full justify-center">
            ...
            <div className="glass rounded-xl p-4">
              ...
              <button onClick={() => window.location.href = './memory'} className="btn-secondary text-xs !py-2 flex items-center gap-2 w-full justify-center !border-aura-orange/30 !text-aura-orange">
            ...
            <div className="glass rounded-xl p-4">
              ...
              <button onClick={handleDeleteEverything} className="btn-secondary text-xs !py-2 flex items-center gap-2 w-full justify-center !border-aura-red/30 !text-aura-red hover:!bg-aura-red/10">
